...
void receiveReport(uint gpio, uint32_t event_mask);
...
//  attachInterrupt(digitalPinToInterrupt(_clockPin), receiveReport, FALLING);
  gpio_set_irq_enabled_with_callback(_clockPin, GPIO_IRQ_EDGE_FALL, true, receiveReport);
  high(_clockPin);
  high(_dataPin);
  return true;
}
...
bool PS2Mouse::writeByte(char data) {
    ...
    while ((gpio_get(_clockPin) == LOW) || (gpio_get(_dataPin) == LOW))
        ;
...
//    Serial.println(c);
    ...
    while (millis() - c < 100) {
      res = (gpio_get(_clockPin) == expectedState) ? true : false;
      if (res) break;
    };
...
void receiveReport(uint gpio, uint32_t event_mask) {
  ...
  val = (gpio_get(PS2_MS_DataPin) ? 1 : 0);
  now_ms = millis();
