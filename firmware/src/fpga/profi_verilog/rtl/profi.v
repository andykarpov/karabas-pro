...
-- Ukraine, EU 2021-2026
-------------------------------------------------------------------------------------------------------------------*/

module profi(
    input wire              clk_bus,        // 28
    input wire              clk_bus_port,   // 84
    input wire              clk_8,
    input wire              areset,
    ...
    output wire             video_ds80,     // profi screen
    output wire             icon_sd,
    output wire             icon_cf,
...
memory memory(
    ...
    .ram_ext                (port_dffd_reg[2:0]), // seg A3 - seg A5
    .trdos                  (dos_act),
...
// Video Pentagon / Profi
wire vid_pff_cs;
wire [7:0] vid_attr;
wire gx0;
wire vid_ispaper;
wire blink;
wire vid_ena = (ds80) ? clk_div2 : clk_div2 && clk_div4; // 14 / 7 MHz
wire vid_turbo = (turbo_mode) ? 1 : 0;
video video(
    .clk                    (clk_bus),
    .ena                    (vid_ena),
    .reset                  (reset),
...
assign video_clk = (ds80) ? clk_div2 : clk_div2 && clk_div4;
...
assign cpu_nmi_n = (btn_nmi && ((~cpu_m1_n && ~cpu_mreq_n && cpu_a_bus[15:14] != 2'b00)) || ds80) ? 1'b0 : 1'b1; 
assign cpu_wait_n = 1'b1;

// max turbo = 14 MHz
wire [1:0] max_turbo = "10";

wire cpu_ena = (btn_wait || ~WAIT_IO) ? 0 : 
    (fdd_wait && turbo_mode == 2'b11 && turbo_mode <= max_turbo) ? 1 : 
    (fdd_wait && turbo_mode == 2'b10 && turbo_mode <= max_turbo) ? ena_div2 : 
    (fdd_wait && turbo_mode == 2'b01 && turbo_mode <= max_turbo) ? ena_div4 : 
    ena_div8;
...
// Z-controller
wire zc_spi_start = ((cpu_a_bus[7:0] == 8'h57) && ~cpu_iorq_n && cpu_m1_n && ~loader_act && ~is_flash_not_sd);
wire zc_wr_en = ((cpu_a_bus[7:0] == 8'h57) && ~cpu_iorq_n && cpu_m1_n && ~cpu_wr_n && ~loader_act && ~is_flash_not_sd);
wire port77_wr = ((cpu_a_bus[7:0] == 8'h77) && ~cpu_iorq_n && cpu_m1_n && ~cpu_wr_n && ~loader_act && ~is_flash_not_sd);
...
//    (~cpu_iorq_n && ~cpu_rd_n && cpu_m1_n && (cpu_a_bus[7:0] == 8'h57) && ~is_flash_not_sd) ? 8'h03 : 
